...
const runSwitchVersionTest = async (po: any, nativeGit: boolean) => {
  await po.setUp({ autoApprove: true, nativeGit });
  await po.sendPrompt("tc=write-index");

  await po.snapshotPreview({ name: `v2${nativeGit ? "-native" : ""}` });
  ...
  await po.snapshotPreview({ name: `v1${nativeGit ? "-native" : ""}` });
